...
1.  **查找成员函数 `Fraction::operator+(double)`**：`Fraction` 类中是否定义了可以直接与 `double` 类型相加的成员函数。
...
除了通过类型转换函数将对象隐式转换为其他类型外，还可以通过 **隐式调用构造函数** 将其他类型的变量转换为当前类类型：
...
public:
    ...
    explicit operator double() const { return (double) (m_numerator * 1.0 / m_denominator); }
...
public:
	// 重载 * 运算符，通过 *sp 解引用 shared_ptr 对象，并返回其所指向的值
    T& operator*() const {  return *px; }
	// 重载 -> 运算符，通过 sp->method() 访问 shared_ptr 对象所指向的对象的成员函数
    T *operator->() const {	return px; }
	shared_ptr(T* p) : px(p) {}
private:
...
func(*sp);	    // 被解释为 func(*px)，解引用 shared_ptr 对象 sp 并将其指向的值传递给 func 函数
sp -> method();	// 被解释为 px -> method()，访问 shared_ptr 对象 sp 指向的对象的成员函数 method
```
...
struct pair {
	...
	pair(const Tl& a,const T2& b) : first(a)，second(b){}
	...
}
```
...
![|650](../../../images/C&C++/Pasted%20image%2020250223154020.png)
...
template <typename T, typename U>
class MyClass<T*, typename U> {};           // 针对指针类型的特化实现

template <typename T, typename U>
class MyClass<T&, typename U> {};           // 针对引用类型的特化实现

template <typename T, typename U>
class MyClass<const T, typename U> {};      // 针对常量类型的特化实现
```
...
为了避免模板参数数量不匹配的问题，我们可以通过`using`关键字为`list`定义一个别名，将其第二个模板参数固定为`allocator<T>`。这样，代码就能够正确编译通过，`mylst2`的成员变量`c`将被特化为`list<std::string>`。

```Cpp
template<typename T>
using LST = list<T, allocator<T>>

MyClass<string, list> mylst1;        // 编译错误
MyClass<string, Lst>  mylst2;        // 编译通过
```
...
类的 **内存布局（memory layout）** 是理解对象在内存中如何存储的关键，尤其是在涉及继承和虚函数的情况下。示例如下：
...
class Derived1: public Base {
    public:
        virtual void vfunc1()  {  cout << "Derived1::vfunc1" << endl; }  // 使用 virtual 重写
        ~Derived1()            { cout << "Derived1 destructor" << endl; }
};

class Derived2: public Base {
    public:
        void vfunc1() override { cout << "Derived2::vfunc1" << endl; }   // 使用 override 重写
...
class Derived1: public Base {
    public:
        virtual void show() { cout << "Derived1 class show function" << endl; }
};

class Derived2: public Base {
    public:
        virtual void show() { cout << "Derived2 class show function" << endl; }
};
```
...
`const`修饰符被视为函数签名的一部分，同一个类中可以同时存在`const`和`non-const`版本的同名成员函数。在这种情况下：
...
class MyClass {
    public:
        MyClass(): value(0) {}
        void setValue(int newValue) { value = newValue; }   // 非常量成员函数
        int  getValue() const       { return value; }       // 常量成员函数

        // 常量成员函数和非常量成员函数重载
        void print() const { cout << "Const function: " << value << endl; }
        void print()       { cout << "Non-const function: " << value << endl; }
