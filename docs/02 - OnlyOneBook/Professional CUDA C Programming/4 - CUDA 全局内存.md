...
#### 1.1. 内存层次结构的优点 Benefits of a Memory Hierarchy
...
### 4. 核函数可达到的带宽 WHAT BANDWIDTH CAN A KERNEL ACHIEVE?

内存延迟指从发起内存请求到数据进入SM寄存器的整个时间，而内存带宽则衡量SM访问内存的速率（单位：GB/s）。上节通过以下方式优化内核性能：

1. 增加线程束数量实现延迟隐藏
2. 优化内存访问合并性提升带宽利用率

但当前内核的内存访问模式存在根本缺陷，难以突破性能瓶颈。本节将以矩阵转置为例，探索如何重构低效内核。

#### 4.1. 内存带宽 Memory Bandwidth

多数核函数对内存带宽高度敏感。可以打个比方：计算核心（SM）就像高效的工人，但由于原料（数据）供应速度不足，导致整体生产效率受限。数据在内存中的组织方式以及线程束（warp）的访问模式，都会影响带宽利用率。通常，我们关注以下两种带宽指标：

- **理论带宽（Theoretical Bandwidth）**：理论带宽是硬件设计所能达到的绝对最大值。例如，对于不启用ECC的Fermi M2090 GPU，其理论峰值带宽为 **117.6 GB/s**
- **有效带宽（Achieved Bandwidth）**：有效带宽是Kernel函数实际运行时测得的带宽，可以通过以下公式计算：
  
  $$
	  \text{有效带宽}=\frac{(读取字节数+写入字节数) \times 10^{-9}}{运行时间}
  $$

此外，吞吐量之间也分为**计算吞吐量**和**内存吞吐量**

- **计算吞吐量（Throughput）**  ：衡量的是计算核心的效率，单位是每秒十亿次浮点运算（GFLOPS），不仅与有效带宽相关，还受到带宽利用率等因素的影响，但最主要的决定因素是设备的运算核心性能
- **内存吞吐量（Memory Throughput）**：表示单位时间内从内存中访问的数据总量，单位为GB/s。虽然数值越大意味着读取的数据越多，但这些数据未必都对计算有用。

#### 4.2. 矩阵转置 Matrix Transpose Problem

矩阵转置是一种常见的线性代数操作，其目标是将矩阵的行与列互换。例如，给定一个 $n×m$ 的矩阵 $A$，其转置矩阵 $B$ 满足：
...
![](../../../images/Professional%20CUDA%20C%20Programming/Matrix%20Transpose.png)

使用串行编程实现矩阵转置非常直观，以下是一个简单的CPU实现：

```C
void transformMatrix2D_CPU(float *MatA, float *MatB, int nx, int ny) {
    for (int j = 0; j < ny; j++) {              // 遍历原始矩阵的行
        for (int i = 0; i < nx; i++) {          // 遍历原始矩阵的列
            MatB[i * ny + j] = MatA[j * nx + i]; // 转置操作
        }
    }
}
```

需要注意的是，无论数据在逻辑上是二维结构（如矩阵）还是多维数组，在内存硬件层面，所有数据都是以一维方式存储的。因此，上述代码通过一维数组实现了矩阵的转置操作。

为了更好地理解矩阵转置的性能瓶颈，我们可以通过内存布局图来分析其访问模式：

![](../../../images/Professional%20CUDA%20C%20Programming/Matrix%20Transpose%20Layout.png)

通过这个图能得出一个结论，转置操作：

- **读操作** ：同一颜色表示一组线程束（warp）能够合并读取的**连续**内存块。
- **写操作** ：不同颜色交错分布，表明写入操作涉及**非连续**内存地址。

交叉访问是导致内存性能下降的主要原因。然而，由于矩阵转置的本质决定了这种交叉访问无法避免，因此我们需要找到一种在这种限制下提升效率的方法。
...
最初的想法可能，按行读取（图一）更有效率，因为写入操作不需要经过一级缓存，直接将数据写入全局内存即可。然而，这种想法忽略了GPU内存层次结构中一级缓存的作用。当线程发起内存加载请求时，GPU会先检查一级缓存中是否已经缓存了所需数据。如果命中，数据直接从缓存中读取，无需访问全局内存，从而减少延迟；如果未命中，则从全局内存加载数据，并将其存储到一级缓存中。

此外，GPU的一级缓存以128字节为单位进行加载，即使一次加载的数据中只有一个被当前线程使用，其余数据也会暂时保留在缓存中。由于一级缓存的容量通常为几KB或更大，这些数据在短时间内不会被替换。因此，在按列读取的情况下，虽然单次读取是非连续的，但后续的读取可能会命中一级缓存。例如，假设第一行的数据被加载到一级缓存中，当线程继续读取同一列的其他元素时，这些数据很可能已经在缓存中，从而避免了额外的全局内存访问。

通过实验对比这两种读取方可以发现，按行读取的优点在于它符合合并访问要求，能够高效利用全局内存带宽，但缺点是**写入操作需要直接访问全局内存，可能导致较高的延迟**。而按列读取虽然单次读取是非连续的，可能降低全局内存带宽利用率，但由于**一级缓存的存在**，整体性能可能反而更高。尤其是在矩阵规模较大、数据复用性较高时，按列读取的优势更加明显。

#### 4.2.1. 为转置核函数设置上限和下限 Setting An Upper and Lower Performance Bound for Transpose Kernels
