...
&= \widetilde{W_k} \cdot X + \widetilde{b_k}
\end{align*} $$
...
\widetilde{W_k} &=  \frac{\gamma_k W_k}{\sqrt {\sigma_k^2 + \epsilon}} = W_k \cdot A \\
\widetilde{b_k} &= \beta_k - \frac{\gamma_k (B - \mu_k)}{\sqrt {\sigma_k^2 + \epsilon}} = (B - \mu_k) \cdot A + \beta_k
\end{align*}$$

从而就将BN层融入到了全连接层的参数中，当不计算偏置项时，可令 $B=0$ 
