...
$$\begin{align*} 
\widehat{A}_n &= \widehat{b}_n + \sum_m \widehat{W}_m \cdot \widehat{x}_m \\
&=\widehat{b}_n+\sum_m (s_W W^{int}_n) \cdot (s_x x_m^{int})\\
&=\widehat{b}_n+ s_W s_x \sum_m W_n^{int} \cdot x_m^{int} \tag{3}\\
\end{align*}$$
...
\widehat{A}_n &= \widehat{b}_n + \sum_m \widehat{W}_{n,m} \cdot \widehat{x}_m \\  
&= \widehat{b}_n + \sum_{m} (s_W W_{n,m}^{int}) \cdot (s_x x_m^{int}) \\  
&= \widehat{b}_n + s_W s_x \sum_{m} W_{n,m}^{int} \cdot x_m^{int}  
\end{align*}  
