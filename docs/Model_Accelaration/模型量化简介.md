...
$$\begin{align*}
\widehat{A}_n&=\widehat{b}_n+\sum_{m} \widehat{W}_{n,m} \widehat{x}_m\\
&=\widehat{b}_n+\sum_{m} (s_W W_{n,m}^{int}) (s_x x_m^{int})\\
&=\widehat{b}_n+s_Ws_x\sum_{m} W_{n,m}^{int} x_m^{int}\\
\end{align*}$$
