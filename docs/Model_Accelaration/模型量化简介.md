...
&= BatchNorm(W_k \cdot X) \\
&= \gamma_k \cdot \hat{x} + \beta \\
&= \gamma_k (\frac{W_k x - \mu_k}{\sqrt {\sigma^2 + \epsilon}}) + \beta_k \\
&= \frac{\gamma_k W_k x}{\sqrt {\sigma_k^2 + \epsilon}} + (\beta_k - \frac{\gamma_k \mu_k}{\sqrt {\sigma_k^2 + \epsilon}}) \\
&= \widetilde{W_k} \cdot X + \widetilde{b}
\end{align*} $$
