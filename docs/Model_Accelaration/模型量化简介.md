...
### 定点数与浮点数 Fixed-point numbers and Floating-point numbers

#### 定点和浮点 Fixed-point and Floating-point 

定点和浮点都是数值的表示，它们区别在于，将整数（integer）部分和小数（fractional）部分分开的点，点在哪里。**定点保留特定位数整数和小数，而浮点保留特定位数的有效数字（significand）和指数（exponent）**。

绝大多数现代的计算机系统采纳了**浮点数**表示方式，这种表达方式利用科学计数法来表达实数。根据IEEE754标准，64位计算机用长实数浮点表示，即用一个**尾数(Mantissa)** 52位，一个**基数(Base)** 11位，一个 **指数(Exponent)** 以及一个表示正负的**符号** 1位来表达实数。具体组成如下：

- 第一部分为 sign 符号位 s，占 1 bit，用来表示正负号，0 或 1 表示。0 代表正数，1 代表负数；
...
比如 123.45 定点数用二进制则表示为 1111011.011100 (小数点后取5位)，用十进制科学计数法可以表示为 $1.2345 \times 10^2$ 浮点数，其中 $1.2345$ 为尾数，10 为基数，2 为指数。

一个8位的计算机，整数部分111111十进制最大只能表示为31；小数部分0.111 最大只能表示0.875，表示的数据范围太小了。当然在16位的计算机中，可以通过增大整数部分位数表示更大的数，增大小数部分的位数可以提高小数精度。但是这种方式对计算机来说开销非常大，所以**大多数计算机并没有选择使用定点数表示小数，而是采用浮点数表示小数**。

单精度浮点类型 float 占用 32bit，所以也称作 FP32；双精度浮点类型 double 占用 64bit

![定点数与浮点数](/images/Model_Accelaration/定点数与浮点数.png)

#### 量化浮点 Quantized Floating-Point
...
神经网络的推理由浮点运算构成。FP32 和 INT8 的值域是 $[(2-2^{32}) \times 2^{127},(2^{23}-2) \times 2^{127}]$ 和 $[-128,127]$，而取值数量大约分别为 $2^{32}$ 和 $2^8$。FP32 取值范围非常广，因此，将网络从 FP32 转换为 INT8 并不像数据类型转换截断那样简单。但是，一般神经网络权重的值分布范围很窄，非常接近零。下图给出了 MobileNetV1 中十层（拥有最多值的层）的权重分布。

![MobileNetV1权重分布](/images/Model_Accelaration/MobileNetV1权重分布.png)
...
> 之所以要插入反量化层（Dequantize），是因为量化技术的早期，只有卷积算子支持量化，但实际网络中还包含其他算子，而其他算子又只支持 FP32 计算，因此需要把 INT8 转换成 FP32。但随着技术的迭代，后期估计会逐步改善乃至消除 Dequantize 操作，达成全网络的量化运行，而不是部分算子量化运行。
...
- **绝对值量化**: 绝对值量化是最简单的量化策略。$V$ 表示要量化的张量，**就是简单的把浮点数直接映射到 int8 的数据范围，量化范围外的值直接映射到定点的 min/max**。这种方法往往会使精度下降较多，因为异常值可能会导致过多的舍入误差 (比如最小值比最大值多出一大截，这部分精度就浪费掉了，如下图所示)
...
![MinMax](/images/Model_Accelaration/MinMax.png)
...
此外，还有以下更为常用的量化范围取值方法：

- KL散度[【模型推理】谈谈几种量化策略：MinMax、KLD、ADMM、EQ - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/414647262)
- ADMM[【模型推理】谈谈几种量化策略：MinMax、KLD、ADMM、EQ - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/414647262)
- EQ[【模型推理】谈谈几种量化策略：MinMax、KLD、ADMM、EQ - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/414647262)
...
- **data free 不使用校准集**: 传统的方法直接将浮点参数转化成量化数，使用上非常简单，但是一般会带来很大的精度损失，但是高通 [DFQ](/docs/Model_Accelaration/Data_Free/DFQ.md) 不使用校准集也得到了很高的精度。
...
- [【模型推理】谈谈几种量化策略：MinMax、KLD、ADMM、EQ - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/414647262)